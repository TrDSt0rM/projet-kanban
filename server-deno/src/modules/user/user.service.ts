...
import { APIErrorCode, APIException, UpdateUserRequest } from "../../shared/types/mod.ts";
...
export class UserService {
    ...
    async getUserByPseudo(pseudo: string) {
        const response = await safeFetch(`${URL_SERVER_TOMCAT}/users?pseudo=${pseudo}`);

        if (!response.ok) {
            if (response.status === 404) {
                throw new APIException(APIErrorCode.NOT_FOUND, 404, "ce pseudo ne correspond à aucun utilisateur");
            }
    ...
    async deleteUser(pseudo: string) {
        const reponse = await safeFetch(`${URL_SERVER_TOMCAT}/users?pseudo=${pseudo}`, {
            method: "DELETE",
        });
