...
export function BoardDetail({
  ...
  // --- AUTOCOMPLETION AVEC FILTRAGE DE SOI-MÊME ---
  useEffect(() => {
      ...
      if (searchPseudo.trim().length > 1) {
        try {
          ...
          if (response.ok) {
            ...
            // FILTRE : On retire l'utilisateur actuel (user.pseudo) des suggestions
            ...
            // Si le pseudo tapé correspond exactement à un utilisateur (autre que soi-même)
            const exactMatch = filteredUsers.find(
              (u: UserDto) => u.pseudo.toLowerCase() === searchPseudo.toLowerCase()
            );
            
            if (exactMatch) {
              setFoundUser(exactMatch.pseudo);
            } else {
              setFoundUser(null);
            }
          }
  ...
  const handleSendInvite = async (e: React.FormEvent) => {
    ...
    // Blocage de sécurité supplémentaire
          ...
          <div className="relative bg-white w-full max-w-md rounded-2xl shadow-2xl p-6 animate-in zoom-in duration-200">
            ...
            <form onSubmit={handleSendInvite}>
              <div className="relative mb-4">
                ...
                {foundUser && !showSuggestions && (
                  <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-1 text-green-600 text-xs font-bold bg-green-50 px-2 py-1 rounded-lg border border-green-100 animate-in fade-in">
